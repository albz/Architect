...
USE class_particle
  ...
  do ibunch=1,sim_par%Nbunches
    ...
    do iparticle=1,size(bunch(ibunch)%part(:))
        ...
        if( abs(bunch(ibunch)%part(iparticle)%cmp(1)-mu_x(1)) > 4.*bunchip%sx_um(ibunch) ) bunch(ibunch)%part(iparticle)%cmp(8)=0.
        if( abs(bunch(ibunch)%part(iparticle)%cmp(2)-mu_y(1)) > 4.*bunchip%sy_um(ibunch) ) bunch(ibunch)%part(iparticle)%cmp(8)=0.
        if( abs(bunch(ibunch)%part(iparticle)%cmp(3)-mu_z(1)) > 4.*bunchip%sz_um(ibunch) ) bunch(ibunch)%part(iparticle)%cmp(8)=0.
 			endif
 		enddo
