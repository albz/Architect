...
USE architect_class_structure
  ...
  do bunch_number=1,sim_par%Nbunches
    ...
    do iparticle=1,size(bunch(bunch_number)%part(:))
        ...
        if( abs(bunch(bunch_number)%part(iparticle)%cmp(1)-mu_x(1)) > 4.*bunch_initialization%bunch_s_x(bunch_number) ) bunch(bunch_number)%part(iparticle)%cmp(8)=0.
        if( abs(bunch(bunch_number)%part(iparticle)%cmp(2)-mu_y(1)) > 4.*bunch_initialization%bunch_s_y(bunch_number) ) bunch(bunch_number)%part(iparticle)%cmp(8)=0.
        if( abs(bunch(bunch_number)%part(iparticle)%cmp(3)-mu_z(1)) > 4.*bunch_initialization%bunch_s_z(bunch_number) ) bunch(bunch_number)%part(iparticle)%cmp(8)=0.
 			endif
 		enddo
